...
class WFC3IR(Instrument):
    """
    class defining the properties of the WFC3IR instrument
    ...
    def __init__(self):
        ...
        """
        self.Nx = 1014 * units.pixel
        self.Ny = 1014 * units.pixel
        self.imshape = np.array([self.Ny, self.Nx])
        self.npix = reduce(lambda x,y:x*y, self.imshape)
        self.center = np.array([self.Ny, self.Nx])/2
        self.pix_scale_x = 135 * units.mas/units.pixel
        self.pix_scale_y = 121 * units.mas/units.pixel
        self.pix_scale = units.quantity([self.pix_scale_y, self.pix_scale_x])
        self.IWA = None
        self.IWApix = None
        self.IWAmask = None
        self.psf_fwhm = None
    ...
    def orient_nup_eleft(self, img, orientat):
        """
        Given a WFC3IR image and header, rotate it so you have Nup and Eleft
        Arguments:
          img: 2-D image
          orientat: ORIENTAT keyword from the WFC3IR header
        """
    ...
    def photometric_conversion(self, filt_name):
        """Photometric conversions for WFC3IR filters"""
        phot_dict = {}
